		...
		if (state == State.CONNECTING) {
			...
			try {
				...
				for (int i = 0; i < 100; i++) {
					...
					lastUpdate = System.currentTimeMillis();
					if (current == null) {
						if (available >= (VirtueTransformer.getInstance().getGameMode().equals(GameMode.RUNESCAPE3) ? 5 : 3)) {
                            inputBuffer.clear();
                            input.read(inputBuffer.array());
                            int index = inputBuffer.get() & 0xff;
                            int file;
                            if (VirtueTransformer.getInstance().getGameMode().equals(GameMode.RUNESCAPE3)) {
                            	file = inputBuffer.getInt() & 0x7fffffff;
                            } else {
                            	file = inputBuffer.getShort();
                            }
