 ...
 * @author : const_
 */
public class UnusedMethodRemovalTransform extends Transformer {
    @Override
    public void transform(List<ClassElement> elements) {
        ...
        entries.add(new EntryPoint(VirtueTransformer.getInstance().getInjector().get("client").findMethod("init", "()V")));
        entries.add(new EntryPoint(VirtueTransformer.getInstance().getInjector().get(VirtueTransformer.getInstance().getInjector().get("client").node().superName).findMethod("start", "()V")));
